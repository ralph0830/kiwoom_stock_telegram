...
- 💰 **자동 매수** (v1.6.0):
  ...
  - 부분 체결 처리: 체결분 수용 + 미체결 자동 취소
  - 폴백 전략: 미체결 시 시장가 재주문 또는 포기 선택 가능
...
# 매수 주문 타입 설정 (v1.6.0)
BUY_ORDER_TYPE=market        # 매수 타입: market (시장가) | limit_plus_one_tick (지정가, 한 틱 위)
BUY_EXECUTION_TIMEOUT=30     # 지정가 체결 확인 타임아웃 (초)
BUY_EXECUTION_CHECK_INTERVAL=5  # 지정가 체결 확인 주기 (초)
...
- 체결 확인: 5초마다 최대 30초
- 3가지 체결 상태 처리:
  - 100% 완전 체결: 실제 평균 매입단가 반영
  - 부분 체결: 체결분 수용 + 미체결 자동 취소
  - 0% 미체결: 폴백 전략에 따라 시장가 재주문 또는 포기
- 일일 1회만 매수 (날짜 기준)
...
시나리오 A (100% 체결):
  주문 100주 → 체결 100주
  평균 매입단가: 10,010원 (실제 체결가)
  → 매도 모니터링 시작

시나리오 B (부분 체결):
  주문 100주 → 체결 60주, 미체결 40주
  미체결 40주 자동 취소
  평균 매입단가: 10,010원
  → 60주로 매도 모니터링 시작
